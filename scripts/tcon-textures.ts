/**
 * @file Collects TCon textures exported from Photoshop and places them in correct locations.
 */
...
import { naturalSort } from '../../dev/lib/utils.js'
...
const actualFiles = fast_glob.sync('E:/tmp/images/*.png')
  .sort(naturalSort)

if (newFiles.length !== actualFiles.length) throw new Error('Cant find file list')

actualFiles.forEach((f: string, i: number) => {
